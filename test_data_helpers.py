...
def createBatch2(samples):
    batch = {}
    batch['encoder_inputs'] = list(reversed(samples))
    batch['encoder_inputs_length'] = len(samples)
    batch['keep_prob_placeholder'] = 1
    return batch
...
def sentence2enco(sentence, word2id):
    ...
    wordIds = []
    for token in tokens:
        if word2id.get(token) == None:
            wordIds.append(3)
        else:
            wordIds.append(word2id.get(token))
    print('tokens:',wordIds)
